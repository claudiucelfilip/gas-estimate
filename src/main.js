...
class Contract {
        ...
        global.imports = this.imports = {
            env: {
                ...
                _payload: payload_ptr => {
                    ...
                    return this.contract_payload_buf;
                },
                _result: (ptr, len) => {
    ...
    async addLife() {
        const go = new Go();
        let result = await WebAssembly.instantiate(wasmCode, {
            ...go.importObject,
            // ...this.imports
        });
     ...
     */
    _test(isEst, wallet, func_name, amount_to_send, ...func_params) {
        ...
        if (isEst && global.life) {
            const res = global.life.run_with_gas_limit(
                this.code,
                "10000000000",
                new Uint8Array(this.vm.instance.exports.memory.buffer),
                func_name
            );
            // console.log(res);
        } else {
    ...
    test(wallet, func_name, amount_to_send, ...func_params) {
        return this._test(false, wallet, func_name, amount_to_send, ...func_params);
    }
    est(wallet, func_name, amount_to_send, ...func_params) {
        return this._test(true, wallet, func_name, amount_to_send, ...func_params);
    }
    ...
    async init() {
        ...
        this.vm = await WebAssembly.instantiate(this.code, this.imports);
...
export {Wavelet, Contract, TAG_NOP, TAG_TRANSFER, TAG_CONTRACT, TAG_STAKE, TAG_BATCH};
